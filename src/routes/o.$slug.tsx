...
export const Route = createFileRoute("/o/$slug")({
  component: OrgPortal,
  head: () => ({ meta: [{ title: "Organization portal" }] }),
});

type Org = { id: string; name: string; tagline: string | null; logo_url: string | null; brand_color: string; accent_color: string };
type Election = { id: string; title: string; description: string | null; status: string; opens_at: string | null; closes_at: string | null };
...
function OrgPortal() {
  ...
  useEffect(() => {
    (async () => {
      const { data: o } = await supabase.from("organizations").select("id,name,tagline,logo_url,brand_color,accent_color").eq("slug", slug).maybeSingle();
      if (o) {
        setOrg(o as Org);
        const { data: e } = await supabase.from("elections").select("id,title,description,status,opens_at,closes_at").eq("organization_id", (o as any).id).in("status", ["open", "closed"]).order("created_at", { ascending: false });
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border text-white" style={{ background: `linear-gradient(135deg, ${org.brand_color}, ${org.accent_color})` }}>
        <div className="mx-auto flex max-w-5xl items-center gap-5 px-6 py-12">
          {org.logo_url && <img src={org.logo_url} alt="" className="h-16 w-16 rounded-xl bg-white/10 object-cover" />}
          <div>
            ...
            <div className="mt-4 space-y-2">
              {closed.map((e) => (
                <div key={e.id} className="flex items-center justify-between rounded-md border border-border px-4 py-3">
                  <div><p className="font-medium">{e.title}</p><p className="text-xs text-muted-foreground">Closed {e.closes_at ? new Date(e.closes_at).toLocaleDateString() : ""}</p></div>
